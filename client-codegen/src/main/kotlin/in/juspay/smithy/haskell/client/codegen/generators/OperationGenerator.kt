...
class OperationGenerator<T : HaskellShapeDirective<OperationShape>>(
    ...
    fun run() {
        ...
        IntoRequestBuilderGenerator(
            ...
            directive.context(),
            httpTrait,
            httpBindingIndex.determineRequestContentType(opShape, "application/json").getOrNull(),
    ...
    inner class OperationErrorGenerator(
        ...
        override fun run() {
            ...
            writer.write(
                ...
                instance #{aeson:N}.FromJSON $operationErrorName
                instance #{utility:N}.OperationError $operationErrorName where
                    mkDeSerializationError = DeSerializationError
                    mkUnexpectedError = UnexpectedError
                    mkUnexpectedStatusError = UnexpectedStatus . #{text:N}.pack . show
        ...
        fun generateStructure() {
            writer.openBlock("data $operationErrorName =", "") {
                ...
                writer.write("| DeSerializationError #{text:T}")
                writer.write("| UnexpectedError #{text:T}")
                writer.write("| UnexpectedStatus #{text:T}")
